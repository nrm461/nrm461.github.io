...
Deck-specific: **controls** (search + `[ FILTERS ]` + Sort, under the nav); **filter rail** (left credits-style facet list; open by default on desktop, full-screen takeover on mobile); **deck grid** (the justified masonry thumbnail wall — no labels; mobile = 3-up soft-crop squares); **detail view** (the still overlay); **shot data** (the technical fields, rendered as part of the credits); **chips** (active-filter `[ … ]` row above the grid).
...
- Detail view: shot-data fields are **part of the credits block** (a `.credits-rest`, `label - value` lines — treated as the same thing as credits, no separate "shot data" header); keyword tags dimmed at the end; the film's other stills show below via `.project-gallery`. No separate "deck project page"; no "more shots from this spot" label.
...
Facets (in the rail): Film, Genre, Color (12 hues + 7 classes), Color Picker, Brightness, Format, Aspect Ratio, Frame Size, Shot Type, Composition, Lighting, Lighting Type, Time of Day, Int/Ext, Number of People, Commercial Flags. Top search box matches film label + keywords.
