...
Deck-specific: **controls** (search + `[ FILTERS ]`, under the nav — **Sort/Random removed 2026-07-18**; on desktop the FILTERS toggle is hidden and the rail is always shown, and the search box width = rail width so its right edge lines up with the rail; on mobile FILTERS sits on its own line below the nav, aligned top-right); **filter rail** (left credits-style facet list; always shown on desktop, full-screen takeover on mobile); **deck grid** (the justified masonry thumbnail wall — no labels; mobile = 3-up soft-crop squares); **detail view** (the still overlay); **shot data** (the technical fields, rendered as part of the credits); **chips** (active-filter `[ … ]` row above the grid).
