...
- Detail view: the big still (`#ovimg`) is **locked to a 16:9 frame** (`aspect-ratio:16/9` + `object-fit:contain` over black) so every big view is the same height — non-16:9 stills pillar/letter-box inside it (2026-07-19). Order under the image: **keyword tags first (`#ovkw`), then credits (`#ovcredits`), then the shot-data/filters list (`#ovmeta`)** — tags moved above credits 2026-07-19. The tags line is **prefixed `tags:`** and **locked to a fixed 2-line height** (`#ovkw{height:2.9em;overflow:hidden}` + `-webkit-line-clamp:2`; renders `tags: —` when empty) so the credits below never jump when stepping between stills (2026.0015). Shot-data fields are still a `.credits-rest` `label - value` block (no separate "shot data" header). The film link reads **`[ watch ]`** and **opens an on-page Vimeo lightbox** (`#vov`), NOT the hidden project page (2026.0015): `build_deck.py` injects `window.DECK_VIMEO` (`{page_slug: vimeoId}` from `data/projects.json`); `openVimeo()` embeds `player.vimeo.com/video/<id>` with the same minimal-chrome params as project pages (autoplay desktop, muted-load mobile); Esc / `[ CLOSE ]` / backdrop closes and stops playback. Falls back to the project-page link only if no Vimeo ID is mapped. The film's other stills show below via `.project-gallery`. No separate "deck project page".
- Search: **multi-term** (2026.0015). Live typing filters as-you-type (`state.q`); pressing **Enter** commits the term as a removable `Search:` chip (`state.qs[]`, AND-combined) and clears the box so terms can stack. Clicking a detail-view keyword tag also pushes a committed chip.
